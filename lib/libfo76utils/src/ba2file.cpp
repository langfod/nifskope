    ...
    const FileInfo& fd) const
{
  ...
  {
    throw NifSkopeError("BA2File: unexpected change to size of loose file %s",
                        fileName);
  }
...
#endif
    {
      throw NifSkopeError("error opening archive file or directory '%s'",
                          fileName);
    }
#if defined(_WIN32) || defined(_WIN64)
...
[[noreturn]] void BA2File::findFileError(const std::string_view& fileName)
{
  ...
  throw NifSkopeError("file %s not found in archive", s.c_str());
}
    ...
    const FileInfo& fd, int mipOffset) const
{
  ...
  {
    throw NifSkopeError("unsupported DXGI_FORMAT 0x%02X",
