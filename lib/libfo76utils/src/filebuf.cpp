    ...
    fileStream(std::uintptr_t(-1))
{
  ...
  try
  {
    ...
    if (std::intptr_t(fileStream) < 0L)
      throw NifSkopeError("error opening input file \"%s\"", fileName);
#if defined(_WIN32) || defined(_WIN64)
    LARGE_INTEGER fsize;
    fsize.QuadPart = -1L;
    if (!GetFileSizeEx(HANDLE(fileStream), &fsize) || fsize.QuadPart < 0L)
      throw NifSkopeError("error seeking input file \"%s\"", fileName);
    ...
    if ((void *) fileBuf == MAP_FAILED)
      throw NifSkopeError("error mapping input file \"%s\"", fileName);
  }
  catch (...)
  {
    if (std::intptr_t(fileStream) >= 0L)
    ...
    bufWritePos(0)
{
  ...
  if (!f)
  {
    if (buf)
      delete[] buf;
    throw NifSkopeError("error opening output file \"%s\"", fileName);
  }
#if defined(_WIN32) || defined(_WIN64)
  fileDesc = _fileno(f);
#else
